...
class Maison_1 extends Construction {
    constructor(x, y) {
        super(x, y, 100, 100, 'assets/Design sans titre/1-removebg-preview.png');
    }
}

class Maison_2 extends Construction {
    constructor(x, y) {
        super(x, y, 100, 100, 'assets/Design sans titre/2-removebg-preview.png');
    }
}

class Hopital extends Construction {
    constructor(x, y) {
        super(x, y, 135, 135, 'assets/Design sans titre/3-removebg-preview.png');
    }
}

class Marche extends Construction {
    constructor(x, y) {
        super(x, y, 200, 200, 'assets/Design sans titre/6-removebg-preview.png');
    }
}
...
function nextTurn() {
    ...
    triggerRandomEvent();
    updateIndicators();
    saveGameState(); // Sauvegarder l'état après un tour
...
function resetGameState() {
    ...
    dynamicObjects.length = 0; // Vider les objets dynamiques
    actionPoints.length = 0; // Réinitialiser les points d'action
    actionPoints.push(
    ...
    updateIndicators();
    saveGameState(); // Sauvegarder l'état réinitialisé
...
function saveGameState() {
    const gameState = {
        ...
        currentTurn,
        actionsThisTurn,
        playerPosition: { x: maire.x, y: maire.y },
        dynamicObjects: dynamicObjects.map(obj => ({
            x: obj.x,
            y: obj.y,
            width: obj.width,
            height: obj.height,
        ...
        actionPoints: actionPoints.map(point => ({
            x: point.x,
            y: point.y,
            radius: point.radius
        }))
...
function loadGameState() {
    ...
    if (savedState) {
        ...
        maire.x = gameState.playerPosition.x;
        maire.y = gameState.playerPosition.y;

        // Restaurer les objets dynamiques
        dynamicObjects.length = 0;
        gameState.dynamicObjects.forEach(obj => {
            dynamicObjects.push(new DynamicObject(obj.x, obj.y, obj.width, obj.height, obj.imageSrc));
        });

        // Restaurer les points d'action
        actionPoints.length = 0;
        gameState.actionPoints.forEach(point => {
            actionPoints.push(new ActionPoint(point.x, point.y));
        });
...
function checkGameEnd() {
    ...
    if (confirm("Voulez-vous recommencer une nouvelle partie ?")) {
        resetGameState(); // Réinitialiser le jeu
    } else {
...
function performAction(action, x, y) {
    ...
    switch (action) {
        ...
        case 'Construire une usine':
            if (budget >= 500) {
                ...
                alert('Pas assez de budget pour construire une usine.');
            }
            break;
        ...
        case 'Créer des espaces verts':
            if (budget >= 200) {
                ...
                dynamicObjects.push(new DynamicObject(x - 45, y - 50, 120, 120, 'assets/Design sans titre/4-removebg-preview.png'));
                removeActionPoint(x, y); // Supprimer le point d'action
        ...
        case 'Installer des Panneaux Solaires':
            if (budget >= 500) {
                ...
                dynamicObjects.push(new DynamicObject(x - 45, y - 50, 120, 120, 'assets/Design sans titre/5-removebg-preview.png'));
                removeActionPoint(x, y);
    ...
    actionsThisTurn++;
    updateIndicators();
    saveGameState(); // Sauvegarder l'état après une action
...
const constructions = [
    ...
    new Immeuble(430, 90),
    new Immeuble(320, 160),
    new Hopital(550, 0),
    new Immeuble(320, 390),
    new Immeuble(320, 510),
];
const actionPoints = [
    new ActionPoint(450, 390),
    new ActionPoint(500, 390),
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadGameState(); // Restaurer l'état du jeu
    updateIndicators();
    drawGame(); // Redessiner le jeu avec les états restaurés
});

//Logique pour garder le jeu intacte même après un rechargement de la page
loadGameState();